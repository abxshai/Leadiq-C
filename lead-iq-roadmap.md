...
**Product**
- [~] **M-CX1 follow-up — reply-content citations + OOO filtering (gated on a replies/threads table).** **Partly addressed 2026-06-09 (M-CX2):** the `crm.smartlead_reply_threads` table landed and is now consumed for an on-demand LLM *summary* per hot/warm lead. **Still open:** (1) per-reply inline citation of the actual reply body in the expand (M-CX2 summarizes the thread but doesn't render each message verbatim), and (2) the **OOO gating of the *hot* rule** — the classifier still tags a lead hot on `reply_time` alone, so an out-of-office auto-reply is still hot (the threads table has no clean sentiment/`lead_category` — null in ingest — to gate on yet). Today the temperature classifier only sees Smartlead `reply_time` — a timestamp, not the reply body. `smartlead_email_stats.lead_category` is 100% null, so we can't tell a genuine interested reply from an **out-of-office auto-reply**: both set `reply_time`, so OOO repliers are currently tagged **hot**. Accepted for now (2026-06-05). **Trigger:** once the ingest team lands a Supabase table holding the actual email replies / threads, (1) cite each reply's content in the "Touchpoint history" expand (per-thread, not just "Replied to · subject"), and (2) gate the **hot** "recent reply" rule on a real-reply signal (sentiment / not-OOO) so auto-replies fall back to warm. Until then, hot from a reply = "they sent any reply in 90 days", OOO included.
...
- [ ] **Page-load latency pass — deferred 2026-06-09 ("good enough for now").** User's "things feel slow" is **page loading, not inference** (sticking with Groq) and **not compute** (Railway Pro won't help — it's server-render / DB / middleware bound). Evidence from dev-server logs: `proxy.ts` (the Supabase auth `getUser()` network call in middleware) ran **~400 ms on every request**, and warm `application-code` on `/leads` was ~1.2 s. Ranked fixes for when we pick this up: (1) **confirm Railway↔Supabase region colocation** — cross-region RTT × several queries-per-page is the likeliest culprit and free to fix; (2) **trim the middleware auth check** — cookie/JWT verification for routine navigation instead of a per-request network `getUser()`, reserving `getUser()` for sensitive ops; (3) **add `loading.tsx` Suspense shells** to the `force-dynamic` pages so they paint instantly and stream data in (largest *perceived*-speed win); (4) **analytics SQL aggregation** (the separate "Analytics performance" item above is a subset of this); (5) **`/leads` specifics** — cache `lead_filter_facets()` (it re-runs three `distinct` scans per load) via `revalidate`, and replace `count: 'exact'` with an estimated count. Start with 1–3 (cheap, hit every page).
