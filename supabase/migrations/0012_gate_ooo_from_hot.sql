-- 0012_gate_ooo_from_hot.sql
-- ---------------------------------------------------------------------------
-- Gate out-of-office auto-replies out of the "hot" rule.
--
-- The Smartlead "replied in the last 90 days -> hot" condition fires on a reply
-- *timestamp*, so an out-of-office auto-reply tags a lead hot exactly like a
-- genuine reply. reply_status (migration 0011) now tells us when the only reply
-- was OOO, so we exclude that from the hot condition. The lead then falls
-- through to WARM via the existing "any outreach/engagement in 6 months" rule —
-- the honest bucket: we reached a live inbox, but there's no real interest.
-- (Not cold — cold means no contact; an OOO proves the address is live.)
--
-- Only the Smartlead recent-reply branch is gated. HubSpot lifecyclestage and
-- HubSpot last-replied are independent signals and stay as-is. Only 'ooo' is
-- gated here; do_not_contact / not_interested are left hot for now (separate
-- product call). Everything else is identical to migration 0011.
-- ---------------------------------------------------------------------------
  ...
  classified as (
    select
      ...
      case
        ...
        -- OOO auto-replies don't count as a genuine reply for the hot rule.
        when j.last_reply >= now() - interval '90 days'
             and coalesce(j.reply_status, '') <> 'ooo'              then 'hot'
