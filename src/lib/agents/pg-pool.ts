...
// `public` = the app's own tables (campaigns, leads, …). Read-only is enforced
// per-transaction in the tool handlers — this list only bounds what's visible.
// `as const` so the tuple's element type stays the literal union, not string[].
export const QUERYABLE_SCHEMAS = ["public"] as const;
